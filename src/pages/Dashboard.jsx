  ...
  return (
    <div className="p-4">
      ...
      <div className="flex justify-end space-x-4 mb-4">
        <button className="btn btn-primary" onClick={() => window.newClassModal.showModal()}>
          New Class
        </button>
      ...
      <div role='tablist' className="tabs tabs-bordered">
        <a
          role='tab'
          className={`tab tab-bordered ${activeTab === 'activeClass' ? 'tab-active' : ''}`}
          onClick={() => setActiveTab('activeClass')}
        >
          Active Class
        </a>
        <a role='tab'
          className={`tab tab-bordered ${activeTab === 'inactiveClass' ? 'tab-active' : ''}`}
          onClick={() => setActiveTab('inactiveClass')}
        >
          Inactive Class
        </a>
        <a role='tab'
          className={`tab tab-bordered ${activeTab === 'unread' ? 'tab-active' : ''}`}
          onClick={() => setActiveTab('unread')}
        >
          Unread
        </a>
        <a role='tab'
          className={`tab tab-bordered ${activeTab === 'submitted' ? 'tab-active' : ''}`}
          ...
          <div className="py-4">
            ...
            <label className="cursor-pointer label">
              ...
              <input
                ...
                onChange={(e) => setNewClassData({ ...newClassData, active: e.target.checked })}
              />
            </label>
          </div>
          <div className="modal-action">
            <button className="btn" onClick={handleNewClassSubmit}>Save</button>
            <button className="btn" onClick={() => window.newClassModal.close()}>Close</button>
          </div>
        </form>
      </dialog>
    </div>
